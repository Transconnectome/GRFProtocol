...
# loading test-set
test.data <- read.csv("../data/test_lin_weak.csv") 
...
fig3a.lin.ours <- ggplot(data.conf, aes(x = Actual, y = Prediction, fill = ratio)) +
...
fig3b.lin.train <- ggplot(data.train.coef, aes(x = True, y = Pred)) +
...
fig3b.lin.test <- ggplot(data.test.coef, aes(x = True, y = Pred)) +
  geom_point(alpha = 0.3, size = 1.5, color = "gray") +
  geom_smooth(method = "lm", se = TRUE, color = "#2C7BB6", linewidth = 1.0) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray40") +
